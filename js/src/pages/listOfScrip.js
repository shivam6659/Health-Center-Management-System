...
import styles from "../../styles/ListOfScip.module.css";
        ...
        {isLoading ? <LoadingPage /> : <div>
            ...
            <div className={styles.container}>
                ...
                <div className={styles.sortingTotalList}>
                    <label className={styles.lblCount}>{paginatedItems.length + " Sellers found"}</label>
                </div>
                <div>
                    <ul>
                        {paginatedItems.map((item, index) => (
                            <li className={styles.card} key={index}>
                                ...
                                <div className={styles.lblConCard}>
                                    <label className={styles.lblCardTop}>{"Posted On"}</label>
                                    <label className={styles.lblCardBottom}>{moment(item?.UpdatedAt).format(ShowDate)}</label>
                                </div>
                                {isStringEqual(item?.Status, "PENDING_REVIEW") ? <Image src={rightTick} width={50} alt='tick' />
                                    : <div className={styles.spanSp}></div>}
